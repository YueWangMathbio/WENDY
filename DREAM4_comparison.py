...
for dataset in range(2):
    ...
    for network in range(5):
        ...
        aurocp, auprp = directed_evaluation(A, covdyn)
        AUROC[0] += aurocp / 5
        AUPR[0] += auprp / 5
        ...
        aurocp, auprp = directed_evaluation(A, sinc_res)
        AUROC[1] += aurocp / 5
        AUPR[1] += auprp / 5
        ...
        nlode = np.abs(nlode)
        aurocp, auprp = directed_evaluation(A, nlode)
        AUROC[2] += aurocp / 5
        AUPR[2] += auprp / 5
        
        genie = GENIE3(data[gt])
        aurocp, auprp = directed_evaluation(A, genie)
        AUROC[3] += aurocp / 5
        AUPR[3] += auprp / 5
