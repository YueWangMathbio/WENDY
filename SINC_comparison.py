...
def wendy_wrap(unused_index, network, sim_num, time_points):
    ...
    auroc_dir_wendy, aupr_dir_wendy = auroc_auprc(A, wendy)
...
def sinc_wrap(unused_index, network, sim_num, time_points):
    ...
    auroc_dir_sinc, aupr_dir_sinc = auroc_auprc(A, sinc)
...
def nlode_wrap(unused_index, network, sim_num, time_points):
    ...
    auroc_dir_nlode, aupr_dir_nlode = auroc_auprc(A, nlode)
...
def genie_wrap(unused_index, network, sim_num, time_points):
    ...
    auroc_dir_genie, aupr_dir_genie = auroc_auprc(A, genie)
...
def dg_wrap(unused_index, network, sim_num, time_points):
    ...
    auroc_dir_dg, aupr_dir_dg = auroc_auprc(A, dg)
...
def main():
    ...
    for sim_num in cell_nums:
        ...
        time_points = np.linspace(0.0, 0.9, 4)
        print('time_points: ', time_points)
        start_time = time.time()            
        ...
        for i in range(2):
            if i == 0:
                time_points = np.linspace(0.0, 0.9, 4)
            if i == 1:
                time_points = np.linspace(0.0, 0.6, 3)
            print('time_points: ', time_points)
            start_time = time.time()
